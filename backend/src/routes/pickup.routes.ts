...
router.get('/', protect, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const [pickups, total] = await Promise.all([
      PickupAssignment.find(queryObj)
        ...
        .limit(Number(limit))
        .lean(),
      PickupAssignment.countDocuments(queryObj),
    ]);
...
router.get(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const pickups = await PickupAssignment.find({
...
router.get('/:id', protect, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const pickup = await PickupAssignment.findById(req.params.id)
      .populate('donation')
      .populate('donor', 'name organization phone address')
      .populate('recipient', 'name organization phone address')
      .populate('volunteer', 'name phone')
    ...
    if (!pickup) {
      throw new ApiError(404, 'Pickup assignment not found');
    }
    ...
    if (!isAuthorized) {
      throw new ApiError(403, 'Not authorized to view this pickup');
    }
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (pickup.volunteer) {
        throw new ApiError(400, 'Pickup already has a volunteer assigned');
      }
      ...
      await FoodDonation.findByIdAndUpdate(pickup.donation, {
        status: 'assigned',
      });
...
router.patch(
  ...
  authorize('volunteer', 'admin'),
  [
    body('status').isIn(['in_transit', 'picked_up', 'delivering', 'delivered', 'completed'])
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        ...
        throw new ApiError(403, 'Not authorized to update this pickup');
      }
      ...
      // Update donation status
      const donationStatus: Record<string, string> = {
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (['delivered', 'completed'].includes(pickup.status)) {
        throw new ApiError(400, 'Cannot cancel completed pickup');
      }
    ...
    try {
      ...
      const availableVolunteers = volunteers
        .filter((v) => {
          ...
          if (!availability) return true; // If no availability set, consider available
          const dayAvailability = (availability as Record<string, { available: boolean; startTime: string; endTime: string }>)[targetDay];
